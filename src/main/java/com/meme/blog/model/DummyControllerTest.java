...
public class DummyControllerTest {
	...
	public String delete(@PathVariable int id) {	// @PathVariable URL 경로에 변수를 넣는경우 (변수 이름)
		try {
			userRepository.deleteById(id);
		} catch (EmptyResultDataAccessException e) {	// 정확한 Exception을 걸어야 문제가 없다.
			return "삭제에 실패하였습니다. 해당 id는 사용할 수 없습니다.";
		}
	 ...
	 */
	// email, password
//	@Transactional // save함수 대신 사용	// 함수 종료시에 자동 commit
	// Transactional 로 save하니까 충돌이 남
...
//	public String join(String username, String passwoard, String email) {	// key = value (약속된 규칙)
	public String join(User user) {	// key = value (약속된 규칙)
		System.out.println("id : " + user.getId());			// Auto-Sequence 
		System.out.println("username : " + user.getUsername());
		System.out.println("passwoard : " + user.getPassword());
		System.out.println("email : " + user.getEmail());
		System.out.println("role : " + user.getRole());	// Default 값 있음
		
		// interface를 통해 (DAO) 바로 DB에 저장	
		user.setRole(RoleType.USER);							// Role Default 값
		userRepository.save(user);
